...
// Backward-compat: this page's data API lives in backend/api/receptionists.php.
if (!empty($_GET['api']) || ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']))) {
    require __DIR__ . '/backend/api/receptionists.php';
}
...
<title>Receptionists - ASCLEPIUS</title>
...
<?php $active = 'receptionists'; require __DIR__ . '/frontend/partials/sidebar.php'; ?>

<main class="ml-[260px] p-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold" style="font-family:'Manrope',sans-serif;">Receptionists</h2>
            <p class="text-sm text-[#3d4946]">Manage the Receptionist registry. Admin access only.</p>
        </div>
        <span class="material-symbols-outlined text-4xl text-[#00685d]">support_agent</span>
    </div>
    ...
    <div class="bg-white rounded-xl border border-[#dfe4e1] p-6 mb-8">
        <h3 class="font-bold mb-4">Add Receptionist</h3>
        <form id="staffForm" class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <input name="deskNo" class="input" placeholder="Desk No.">
...
<script>
    const API = 'backend/api/receptionists.php';
    ...
    async function loadStaff() {
        ...
        tbody.innerHTML = rows.map(r => `
            <tr class="border-b border-[#eaefec] hover:bg-[#f6faf8]">
                ...
                <td class="px-5 py-3">${esc(r.deskNo || '-')}</td>

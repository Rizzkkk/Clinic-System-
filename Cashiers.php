...
// Backward-compat: this page's data API lives in backend/api/cashiers.php.
if (!empty($_GET['api']) || ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']))) {
    require __DIR__ . '/backend/api/cashiers.php';
}
...
<title>Cashiers - ASCLEPIUS</title>
...
<?php $active = 'cashiers'; require __DIR__ . '/frontend/partials/sidebar.php'; ?>

<main class="ml-[260px] p-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold" style="font-family:'Manrope',sans-serif;">Cashiers</h2>
            <p class="text-sm text-[#3d4946]">Manage the Cashier registry. Admin access only.</p>
        </div>
        <span class="material-symbols-outlined text-4xl text-[#00685d]">point_of_sale</span>
    </div>
...
<script>
    const API = 'backend/api/cashiers.php';
    ...
    async function loadStaff() {
        ...
        tbody.innerHTML = rows.map(r => `
            <tr class="border-b border-[#eaefec] hover:bg-[#f6faf8]">
                ...
                <td class="px-5 py-3">${esc(r.counterNo || '-')}</td>

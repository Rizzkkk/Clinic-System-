...
if (!empty($_GET['api']) || ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']))) {
    require __DIR__ . '/backend/api/xray_studies.php';
}
require_once __DIR__ . '/backend/auth/rbac.php';
require_module_access('xray_studies');
$canWrite = can_access('xray_studies', 'write');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>X-Ray Studies - ASCLEPIUS</title>
...
<?php $active = 'xray_studies'; require __DIR__ . '/frontend/partials/sidebar.php'; ?>

<main class="ml-[260px] p-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold" style="font-family:'Manrope',sans-serif;">X-Ray Studies</h2>
            <p class="text-sm text-[#3d4946]">Record and review x-ray studies per patient.</p>
        </div>
        <span class="material-symbols-outlined text-4xl text-[#00685d]">radiology</span>
    </div>
...
<div class="bg-white rounded-xl border border-[#dfe4e1] p-6 mb-8">
        <h3 class="font-bold mb-4">Add Study</h3>
        <form id="recForm" class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <input name="studyDate" type="date" class="input" title="Study Date" aria-label="Study Date">
            <input name="bodyPart" list="xrayViews" class="input" placeholder="Pick or type, e.g. Chest PA">
            <datalist id="xrayViews"><option value="Chest PA"></option><option value="Chest Lateral"></option><option value="Skull"></option><option value="Cervical Spine"></option><option value="Abdomen"></option><option value="Extremity"></option><option value="Pelvis"></option></datalist>
            <input name="radiologist" class="input" placeholder="Radiologist">
            <textarea name="findings" class="input md:col-span-3" rows="2" placeholder="Findings"></textarea>
            <textarea name="impression" class="input md:col-span-3" rows="2" placeholder="Impression"></textarea>
            <textarea name="remarks" class="input md:col-span-3" rows="2" placeholder="Remarks (study-specific notes)"></textarea>
            <select name="status" class="input"><option value="">Status</option><option>Released</option><option>Pending</option></select>
            <label class="md:col-span-3 text-xs font-bold uppercase tracking-wider text-[#6d7a77]">X-ray image (optional) &mdash; JPG, PNG, WEBP, or PDF, max 10 MB
                <input name="xrayImage" type="file" accept=".jpg,.jpeg,.png,.webp,.pdf,image/*,application/pdf" class="input mt-1">
            </label>
    ...
    <div class="bg-white rounded-xl border border-[#dfe4e1] overflow-hidden">
        <table class="w-full text-sm">
            <thead><tr class="text-left text-xs uppercase tracking-wider text-[#6d7a77] border-b border-[#dfe4e1]">
                ...
                <th class="px-5 py-3">Body Part</th>
                <th class="px-5 py-3">Radiologist</th>
...
<script>
    const API = 'backend/api/xray_studies.php';
    ...
    async function loadRecords() {
        ...
        document.getElementById('recRows').innerHTML = rows.map(r => `
            <tr class="border-b border-[#eaefec] hover:bg-[#f6faf8]">
                ...
                <td class="px-5 py-3">${esc(r.studyDate || '-')}</td>
                <td class="px-5 py-3">${esc(r.bodyPart || '-')}</td>
                <td class="px-5 py-3">${esc(r.radiologist || '-')}</td>
                <td class="px-5 py-3">${esc(r.status || '-')}</td>
                <td class="px-5 py-3 text-right">
                    ${r.imagePath ? `<a href="backend/api/xray_image.php?id=${Number(r.id)}" target="_blank" rel="noopener" class="text-[#00685d] hover:underline text-xs font-bold mr-3">View image</a>` : ''}
                    ${CAN_WRITE ? `<button onclick="startEdit(${Number(r.id)})" class="text-[#00685d] hover:underline text-xs font-bold mr-3">Edit</button><button onclick="del(${Number(r.id)})" class="text-red-700 hover:underline text-xs font-bold">Delete</button>` : ''}
                </td>
            </tr>`).join('');
    }

...
-- Individual tests within a laboratory order (one order -> many tests). The parent
-- laboratory_results row carries the shared fields (patient, date, ordering doctor, remarks).
CREATE TABLE IF NOT EXISTS laboratory_result_items (
  ...
  CONSTRAINT laboratory_result_item_fk FOREIGN KEY (resultId) REFERENCES laboratory_results(id) ON DELETE CASCADE
);

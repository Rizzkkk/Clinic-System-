...
- Staff account creation (`Staff Accounts.php`, admin-only) hashes with `password_hash($pw, PASSWORD_DEFAULT)`, rejects
...
| S-9 | MED | **No rate limiting** on login. *Password policy addressed:* `Portal Register.php` and `Staff Accounts.php` both enforce a minimum of 8 characters. There is still **no throttling or lockout anywhere**. Severity raised from LOW because portal accounts are internet-facing and sit in front of PHI. | `login.php` | Add throttling/lockout on repeated login failures. |
| S-20 | done | **Public staff self-registration removed.** `register.php` let anyone create a `users` row on a system holding PHI; the account was role-less, but it was an unauthenticated write and a standing privilege-escalation target (one admin mis-click away from access). Staff logins are now created only by an admin in `Staff Accounts.php` (RBAC module `staff_accounts`, empty read/write role lists = admin only), which also assigns roles to legacy `pending` accounts. The role predicate on the update keeps it off portal rows, and an admin cannot change their own role (lockout). | `Staff Accounts.php`, `rbac.php`, `login.php` | Done. First admin is seeded in SQL (deployment.md step 5). |
...
| S-15 | LOW | **Account enumeration on portal signup** - "an account with this email already exists" is an existence oracle on a *health* service. Consistent with the staff-account form's behaviour. | `Portal Register.php` | Accepted for now. Fix by always showing the same neutral message and emailing the address instead. |
| S-13 | done | **Input validation** — shared `backend/lib/validation.php` (`is_valid_name`/`is_valid_email`) rejects malformed names (digits/symbols) and emails **server-side before any DB write** in `Staff Accounts.php` and all five registration APIs (add + update); matching HTML5 `pattern` gives client-side feedback. Improves data integrity for PHI records. | validation.php, Staff Accounts.php, api/* | Done. Names/email only; free-text fields (address, notes) unchanged. Password policy (S-9) still open. |

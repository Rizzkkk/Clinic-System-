...
$laboratoryResult = $conn->query('
    SELECT lr.id, lr.patientId, lr.orderedBy, lr.testType, lr.testDate, lr.results, lr.referenceRange, lr.remarks, lr.abnormalFlag,
...
<div><label class="block text-label-bold text-on-surface-variant mb-1 uppercase">Test Type</label>
<input name="testType" list="labTests" required placeholder="Pick or type, e.g. HbA1c" class="w-full bg-white border border-outline-variant rounded-lg py-2 px-3 text-body-md focus:ring-2 focus:ring-primary outline-none"/>
<datalist id="labTests"><option value="HbA1c"></option><option value="CBC"></option><option value="FBS"></option><option value="Lipid Panel"></option><option value="Urinalysis"></option><option value="Creatinine"></option><option value="TSH"></option><option value="SGPT/ALT"></option><option value="Blood Typing"></option></datalist></div>
...
<div><label class="block text-label-bold text-on-surface-variant mb-1 uppercase">Reference Range</label>
<input name="referenceRange" placeholder="e.g. 4.5-11.0" class="w-full bg-white border border-outline-variant rounded-lg py-2 px-3 text-body-md focus:ring-2 focus:ring-primary outline-none"/></div>
<div><label class="block text-label-bold text-on-surface-variant mb-1 uppercase">Abnormal?</label>
<select name="abnormalFlag" class="w-full bg-white border border-outline-variant rounded-lg py-2 px-3 text-body-md focus:ring-2 focus:ring-primary outline-none"><option value="N">Normal</option><option value="Y">Abnormal</option></select></div>
<div class="md:col-span-3"><label class="block text-label-bold text-on-surface-variant mb-1 uppercase">Results</label>
<textarea name="results" rows="2" placeholder="Result values / findings" class="w-full bg-white border border-outline-variant rounded-lg py-2 px-3 text-body-md focus:ring-2 focus:ring-primary outline-none"></textarea></div>
...
<td class="px-4 py-4 text-body-sm text-on-surface-variant"><?php echo h($result['testType'] ?: 'Laboratory test'); ?><?php if (!empty($result['remarks'])): ?><div class="text-xs text-on-surface-variant/70 mt-0.5"><?php echo h($result['remarks']); ?></div><?php endif; ?></td>
...
<script>
    ...
    function labStartEdit(id) {
        ...
        if (!r || !f) return;
        labEditingId = id;
        ['patientId','orderedBy','testType','testDate','referenceRange','remarks','abnormalFlag','results'].forEach(k => {
    ...
    labLoadStats(); labLoadPatients(); labLoadDoctors();
</script>
</body></html>
